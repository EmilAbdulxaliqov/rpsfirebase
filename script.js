...
$("#playerPanel1").on("click", ".panel-option", function(event) {
	event.preventDefault();
	...
	if (player1 && player2 && (yourPlayerName === player1.name) && (turn === 1) ) {
		...
		var choice = $(event.target).attr('class').trim().split('-');
		// choice[choice.length-1][0].split() = choice[choice.length-1][0].toUpperCase();
		var newChoice = choice[choice.length - 1].split('');
		// newChoice[newChoice.length - 1][0] = newChoice[newChoice.length - 1][0].toUpperCase();
		newChoice[0] = newChoice[0].toUpperCase();
		console.log(choice,newChoice.join(''));
		// Record the player choice into the database
		player1Choice = newChoice.join('');
		database.ref().child("players/player1/choice").set(newChoice.join(''));
...
$("#playerPanel2").on("click", ".panel-option", function(event) {
	event.preventDefault();
	...
	if (player1 && player2 && (yourPlayerName === player2.name) && (turn === 2) ) {
		...
		console.log(choice,newChoice.join(''));
		// Record the player choice into the database
		player2Choice = newChoice.join('');
		database.ref().child("players/player2/choice").set(newChoice.join(''));
